...
const CheckoutForm = () => {
  ...
  // database'de olduğu gibi burda da stripe'la iletişime geçiyoruz.
  const stripe = useStripe()
  const elements = useElements()

  const createPaymentIndent = async () => {
    try {
      const { data } = await axios.post(
        '/.netlify/functions/create-payment-intent',
        JSON.stringify({ cart, total_price, shipping_fee })
      )
  ...
  return (
    <div>
      {succeeded ? (
        ...
        <article>
          <h4>Hello, {isUser && isUser.name}</h4>
